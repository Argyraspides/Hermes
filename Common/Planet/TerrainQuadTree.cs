...
//
//     ConcurrentQueue<TerrainQuadTree> m_splitQueue = new ConcurrentQueue<TerrainQuadTree>();
//     ConcurrentQueue<TerrainQuadTree> m_mergeQueue = new ConcurrentQueue<TerrainQuadTree>();
//
...
public partial class TerrainQuadTree : Node
{
    ...
    // Update the quadtree every X milliseconds
    public int m_quadTreeUpdateInterval = 250;

    public ConcurrentQueue<TerrainQuadTreeNode> m_splitQueueNodes = new ConcurrentQueue<TerrainQuadTreeNode>();
    public ConcurrentQueue<TerrainQuadTreeNode> m_mergeQueueNodes = new ConcurrentQueue<TerrainQuadTreeNode>();
    public ConcurrentQueue<TerrainQuadTreeNode> m_cullQueueNodes = new ConcurrentQueue<TerrainQuadTreeNode>();

    TerrainQuadTreeUpdater m_quadTreeUpdater;

    public Vector3 cachedCamPos;

    public bool canUpdateQuadTree = false;
    ...
    public TerrainQuadTree(PlanetOrbitalCamera camera, int maxNodes = 15000, int minDepth = 6, int maxDepth = 20)
    {
        ...
        m_quadTreeUpdater.DoneDFS += OnDoneDFS;
    }

    void OnDoneDFS()
    {
        canUpdateQuadTree = true;
    }
    ...
    {
        cachedCamPos = m_camera.Position;

        if (canUpdateQuadTree)
        {
            while (m_cullQueueNodes.Count > 0)
            {
                TerrainQuadTreeNode n;
                m_cullQueueNodes.TryDequeue(out n);
                CullUnusedNodes(n);
            }

            while (m_splitQueueNodes.Count > 0)
            {
                TerrainQuadTreeNode n;
                m_splitQueueNodes.TryDequeue(out n);
                Split(n);
            }

            while (m_mergeQueueNodes.Count > 0)
            {
                TerrainQuadTreeNode n;
                m_mergeQueueNodes.TryDequeue(out n);
                MergeChildren(n);
            }

            canUpdateQuadTree = false;
...
{
    public TerrainQuadTree m_terrainQuadTree;
    public Thread m_updateQuadTreeThread;

    [Signal]
    public delegate void DoneDFSEventHandler();

    public bool canDFS = true;
    ...
    private void UpdateQuadTreeThreadFunction()
    {
        while (m_terrainQuadTree.m_isRunning)
        {
            try
            {
                if (m_terrainQuadTree.m_rootNode != null && canDFS)
                {
                ...
                Thread.Sleep(m_terrainQuadTree.m_quadTreeUpdateInterval);
            }
            catch (Exception ex)
            {
                GD.PrintErr($"Error in quadtree update thread: {ex}");
    ...
    private void UpdateQuadTree(TerrainQuadTreeNode node)
    {
        ...
        {
            m_terrainQuadTree.m_splitQueueNodes.Enqueue(node);
            return;
        }

        if (ShouldChildrenMerge(node))
        {
            m_terrainQuadTree.m_mergeQueueNodes.Enqueue(node);
            return;
        }
    ...
    private bool ShouldSplit(TerrainQuadTreeNode node)
    {
        ...
        if (node.Depth >= m_terrainQuadTree.m_maxDepth || !NodeInView(node))
        {
            return false;
        }

        float distToCam = node.Pos.DistanceTo(m_terrainQuadTree.cachedCamPos);
    ...
    private bool ShouldMerge(TerrainQuadTreeNode node)
    {
        ...
        if (node.Depth <= m_terrainQuadTree.m_minDepth + 1 || !NodeInView(node))
        {
            return false;
        }
